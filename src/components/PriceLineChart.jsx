...
export default function PriceLineChart({ points = [], compact = false }) {
    ...
    return <section className={`price-chart-panel ${compact ? 'compact' : ''} ${rising ? 'rising' : 'falling'}`}>
        {!compact && <header>
            ...
            <div className="chart-current-price"><strong>{formatPrice(lastPoint.price)}</strong><b>{changeRate >= 0 ? '+' : ''}{changeRate.toFixed(2)}%</b></div>
        </header>}
        ...
        {!compact && <footer><span>{formatChartDate(firstPoint.timestamp, intraday)}</span><b>{intraday ? '오늘 · 15분 지연' : '최근 거래일'}</b><span>{formatChartDate(lastPoint.timestamp, intraday)}</span></footer>}
    </section>
}
